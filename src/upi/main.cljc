(ns upi.main
  "Kotodama WASM entrypoint for the Upi clean-room actor (L4) — Clojure port.
  ...
  facts (`upi.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"upi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "upi-compat" :tier tier :entities entities} 200])
